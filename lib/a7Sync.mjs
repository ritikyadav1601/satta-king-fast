...
export async function syncA7Results({
  ...
  const scraped = parseA7Results(html);
  ...
  for (const siteGameName of targetGames) {
    ...
    if (!isResult(source.today)) {
      report.skipped.push(`${siteGameName}: pending/invalid result (${source.today || "blank"})`);
      continue;
    }

    report.matched++;

    const existing = await GameResult.findOne({ game: game._id, resultDate }).lean();
    if (existing?.result === source.today) {
      report.unchanged++;
      continue;
    }
...
async function getTargetGames({ activeByName, resultDate, smart, windowMinutes, now }) {
  ...
  const gameIds = activeWindowGames
    .map((gameName) => activeByName.get(normalizeName(gameName))?._id)
    .filter(Boolean);
