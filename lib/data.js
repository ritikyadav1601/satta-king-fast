...
import { addDays, daysInMonth, istDate, sanitizeColumn } from "@/lib/utils";
...
function hasTodayResult(game) {
  const value = String(game.second || "").trim().toUpperCase();
...
export async function getGamesWithTodayResults() {
  ...
  return games.map((game) => ({
    ...game,
    first: yesterdayMap.get(String(game._id))?.result || "XX",
    second: todayMap.get(String(game._id))?.result || "XX"
  }));
}
...
export async function getTopGames(games) {
  ...
  const released = byTime.filter(hasTodayResult);
  const pastReleased = released.filter((game) => timeToMinutes(game.resultTime) <= now);
  const latestResult = pastReleased.at(-1) || released.at(-1) || byTime.filter((game) => timeToMinutes(game.resultTime) <= now).at(-1) || byTime.at(-1);
  const upcoming =
    byTime.find((game) => !hasTodayResult(game) && timeToMinutes(game.resultTime) > now && String(game._id) !== String(latestResult?._id)) ||
    byTime.find((game) => timeToMinutes(game.resultTime) > now && String(game._id) !== String(latestResult?._id)) ||
    byTime.find((game) => !hasTodayResult(game) && String(game._id) !== String(latestResult?._id)) ||
    byTime.find((game) => String(game._id) !== String(latestResult?._id));

  return [latestResult, upcoming].filter(Boolean);
}

export async function getMonthlyRows({ year, month, untilToday = true } = {}) {
  ...
  const games = (await getActiveGames("time")).sort((a, b) => monthlyChartOrder(a) - monthlyChartOrder(b));
  ...
  for (const result of results) {
    ...
    byDate.get(result.resultDate).set(String(result.game), result.result || "-");
  }
...
export async function getYearChartRows(gameSlug, year) {
  ...
  for (const result of results) {
    ...
    if (rows[day - 1]) rows[day - 1][months[month - 1]] = result.result || "-";
  }
